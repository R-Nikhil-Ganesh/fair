...
const BACKEND_URL = functions.config().backend?.url || process.env.BACKEND_URL || "";

// Trigger: fires when a file is finalized in Firebase Storage
// Pattern: audits/{uid}/{auditId}/data.csv
export const onAuditFileUploaded = functions.storage
  .object()
  .onFinalize(async (object) => {
    ...
    try {
      const response = await axios.post(`${BACKEND_URL}/api/audit/start`, {
        audit_id: auditId,
        csv_path: filePath,
...
export const cleanupOldAuditFiles = functions.pubsub
