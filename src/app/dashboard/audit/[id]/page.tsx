...
// ── Clickable launcher row item ───────────────────────────────────────────────
function LaunchRow({
  icon, title, sub, accentColor, badge, onClick, children,
}: {
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          {badge && (
            <span style={{
              fontSize: "0.65rem", fontWeight: 700, borderRadius: 999, padding: "0.1rem 0.4rem",
              background: "rgba(239,68,68,0.15)", color: "#f87171",
              border: "1px solid rgba(239,68,68,0.3)",
            }}>{badge}</span>
          )}
          {children}
          {onClick && <span style={{ fontSize: "0.75rem", color: "#52525b" }}>›</span>}
        </div>
      </button>
    </div>
  );
...
export default function AuditResultsPage() {
  ...
  // Fairness modal controlled from page so the LaunchRow button opens it
  const [fairnessOpen,  setFairnessOpen]  = useState(false);
  // AuditInsights is driven by its own internal state; we pass an external trigger
  ...
  if (loading) return <div style={{ padding: "2rem", textAlign: "center", color: "#71717a" }}>Loading audit results…</div>;
  ...
  const hasCF        = Boolean(report.modelAudit?.counterfactual_data?.narrative);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem", maxWidth: "64rem", margin: "0 auto" }}>
      ...
      {isProcessing && (
        <div style={{ borderRadius:"1rem", border:"1px solid rgba(245,158,11,0.4)", background:"rgba(245,158,11,0.08)", display:"flex", gap:"0.75rem", padding:"1rem" }}>
          ...
          <div>
            ...
            <p style={{ fontSize:"0.82rem", color:"#a1a1aa", marginTop:"0.25rem" }}>Metrics and narrative will appear once Firestore receives results.</p>
          </div>
        </div>
      )}
      ...
      {isFailed && (
        <div style={{ borderRadius:"1rem", border:"1px solid rgba(239,68,68,0.4)", background:"rgba(239,68,68,0.08)", padding:"1rem" }}>
          ...
          <p style={{ fontSize:"0.82rem", color:"#a1a1aa", marginTop:"0.25rem" }}>Use Settings → Cloud Diagnostics to verify backend dependencies.</p>
        </div>
      )}
      ...
      <div style={CARD}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"0.6rem" }}>
          ...
          <LaunchRow
            ...
            title="Fairness Metrics & Approval Rates"
            sub={isProcessing ? "Pending…" : hasFlag ? `${flaggedCount} metric${flaggedCount > 1 ? "s" : ""} flagged — action required` : "All thresholds passed · Disparate impact, parity, odds + approval chart"}
            accentColor="#3b82f6"
          ...
          {report.modelAudit && (
            <LaunchRow
              icon={<Cpu size={16} style={{ color:"#a5b4fc", flexShrink:0 }} />}
              title="Model Analysis"
              sub={`${report.modelAudit.model_type?.toUpperCase() || "Model"} · Accuracy ${report.modelAudit.model_accuracy >= 0 ? `${(report.modelAudit.model_accuracy * 100).toFixed(1)}%` : "—"}${hasCF ? ` · CF flip rate ${(report.modelAudit.counterfactual_data!.flip_rate * 100).toFixed(0)}%` : ""}`}
              accentColor="#818cf8"
            >
              {/* ModelStatsModal brings its own trigger button */}
      ...
      {/* FairnessMetricsModal — controlled externally */}
      <FairnessMetricsModal
        report={report}
        externalOpen={fairnessOpen}
        onExternalClose={() => setFairnessOpen(false)}
      />

      {/* AuditInsights — externally triggered for Summary / Action */}
      <AuditInsights
        report={report}
        externalOpen={insightsOpen}
        onExternalClose={() => setInsightsOpen(null)}
      />
    </div>
  );
}
